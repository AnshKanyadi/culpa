"""
Core recording engine for Culpa.

The CulpaRecorder captures every LLM call, tool invocation, file change,
...
class CulpaRecorder:
    ...
    def start_session(self, name: str, metadata: Optional[dict[str, Any]] = None) -> str:
        ...
        with self._lock:
            ...
            logger.info(f"Culpa session started: {session_id} — {name!r}")
            return session_id
    ...
    def end_session(self) -> Session:
        ...
        with self._lock:
            if self._session is None:
                raise RuntimeError("No active Culpa session. Call start_session() first.")
            ...
            logger.info(
                f"Culpa session ended: {session.session_id} — "
    ...
    def fail_session(self, error: Optional[str] = None) -> Session:
        ...
        with self._lock:
            if self._session is None:
                raise RuntimeError("No active Culpa session.")
...
def get_recorder() -> Optional[CulpaRecorder]:
    """Get the global Culpa recorder instance."""
...
) -> Generator[CulpaRecorder, None, None]:
    """
